...
!EP   Performance difference unchecked
      do ilvl=1,level
!JR   Swapping in explicit do loops for OpenMP
        !$OMP PARALLEL DO DEFAULT(none) PRIVATE(i, k) SHARED(zs, ze, xs, xe, ys, ye, ilvl, in, level)
        do k = zs-level, ze+level
          do i = xs, xe
            in(i,ye+ilvl,k) = in(i,ys+(ilvl-1),k)
            in(i,ys-ilvl,k) = in(i,ye-(ilvl-1),k)
          end do
        end do
        !$OMP END PARALLEL DO
      end do
   ...
   ! *** top/bottom ***
     ...
     if(tag_t.ne.tag_b) then
      ...
      do ilvl=1,level
!JR   Swapping in explicit do loops for OpenMP
        !$OMP PARALLEL DO DEFAULT(none) PRIVATE(i, j) SHARED(zs, ze, xs, xe, ys, ye, ilvl, in, level)
        do j = ys, ye+level
          do i = xs, xe
            in(i,j,ze+ilvl) = in(i,j,zs+(ilvl-1))
            in(i,j,zs-ilvl) = in(i,j,ze-(ilvl-1))
          end do
        end do
        !$OMP END PARALLEL DO
      end do
