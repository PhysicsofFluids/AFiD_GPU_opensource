...
!=======================================================================
    ...
    ! then call MPI send/receive to exchange halo data
     ...
     if(tag_n.ne.tag_s) then
#ifdef USE_CUDA
       istat = cudaMemcpy2D( work1_r(               1), ilength, in(xs,ys          ,zs), ijump, ilength, icount )
       istat = cudaMemcpy2D( work1_r(ilength*icount+1), ilength, in(xs,ye-(level-1),zs), ijump, ilength, icount )

       ! receive from south
       call MPI_IRECV(work2_r(               1), ilength*icount, data_type, &
            neighbour(1,4), tag_s, DECOMP_2D_COMM_CART_X, &
            requests(1), ierror)
       ! receive from north
       call MPI_IRECV(work2_r(ilength*icount+1), ilength*icount, data_type, &
       ...
       ! send to south
       call MPI_ISSEND(work1_r(               1), ilength*icount, data_type, &
            neighbour(1,4), tag_s, DECOMP_2D_COMM_CART_X, &
            requests(3), ierror) 
       ! send to north 
       call MPI_ISSEND(work1_r(ilength*icount+1), ilength*icount, data_type, &
            neighbour(1,3), tag_n, DECOMP_2D_COMM_CART_X, &
            requests(4), ierror) 

       call MPI_WAITALL(4, requests, status, ierror)

       istat = cudaMemcpy2D( in(xs,ys-level,zs), ijump, work2_r(               1), ilength, ilength, icount )
       istat = cudaMemcpy2D( in(xs,ye+1    ,zs), ijump, work2_r(ilength*icount+1), ilength, ilength, icount )
...
!EP   Performance difference unchecked
      if (level .ne. 1)  print *,"This is not supported"


     !$cuf kernel do(2) <<<*,*>>> 
      do j=lbound(in,3),ubound(in,3) 
       do i=lbound(in,1),ubound(in,1) 
       in(i,ye+1,j) = in(i,ys,j)
       in(i,ys-1,j) = in(i,ye,j)
      end do
     end do
     endif
     ...
     if(tag_t.ne.tag_b) then
#ifdef USE_CUDA
       istat = cudaMemcpy( work1_r(1       ), in(xs,ys,zs          ), icount )
       istat = cudaMemcpy( work1_r(1+icount), in(xs,ys,ze-(level-1)), icount ) 
       ! receive from bottom
       call MPI_IRECV(work2_r(1), icount, data_type, &
            neighbour(1,6), tag_b, DECOMP_2D_COMM_CART_X, &
            requests(1), ierror)
       ! receive from top
       call MPI_IRECV(work2_r(1+icount), icount, data_type, &
       ...
       ! send to bottom
       call MPI_ISSEND(work1_r(1), icount, data_type, &
            neighbour(1,6), tag_b, DECOMP_2D_COMM_CART_X, &
            requests(3), ierror)
       ! send to top
       call MPI_ISSEND(work1_r(1+icount), icount, data_type, &
            neighbour(1,5), tag_t, DECOMP_2D_COMM_CART_X, &
            requests(4), ierror)
       call MPI_WAITALL(4, requests, status, ierror)

       istat = cudaMemcpy( in(xs,ys,zs-level), work2_r(1       ), icount )
       istat = cudaMemcpy( in(xs,ys,ze+1    ), work2_r(1+icount), icount )
#else
       ! receive from bottom
     ...
     !$cuf kernel do(2) <<<*,*>>> 
      do j=lbound(in,2),ubound(in,2) 
       do i=lbound(in,1),ubound(in,1) 
       in(i,j,ze+1) = in(i,j,zs)
       in(i,j,zs-1) = in(i,j,ze)
      end do
     end do
     endif
