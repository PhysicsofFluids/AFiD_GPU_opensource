...
#ifdef USE_CUDA
  subroutine transpose_y_to_x_real_d(src, dst, decomp)
...
call nvtxStartRange("tranYX",5)
    ...
    do iter=0,dims(1)-1
       ...
       if(iter==0) then ! if exchanging with self
         ...
         istat = cudaMemcpy2DAsync( work1_r_d(pos), s1*(decomp%y1dist(m)), src(1,decomp%y1idx(m),1), s1*s2, s1*(decomp%y1dist(m)), s3, stream=a2a_comp )
         istat = cudaEventRecord( a2a_event(iter+dims(1)), a2a_comp )
         istat = cudaStreamWaitEvent( a2a_d2h, a2a_event(iter+dims(1)), 0)
         istat = cudaMemcpyAsync( work1_r(pos), work1_r_d(pos), decomp%y1cnts(m), a2a_d2h )
    ...
    do iter=1,dims(1)-1
       m = IEOR(col_rank,iter)
      call MPI_IRECV( work2_r(decomp%x1disp(m)+1), decomp%x1cnts(m), real_type, m, 0, DECOMP_2D_COMM_COL, a2a_requests(iter), ierror)
    end do
...
call nvtxStartRangeAsync("MPI",iter)
       call MPI_SEND( work1_r(decomp%y1disp(m)+1), decomp%y1cnts(m), real_type, m, 0, DECOMP_2D_COMM_COL, ierror)
call nvtxEndRangeAsync
       call MPI_WAIT(a2a_requests(iter), a2a_status(1,iter), ierror)
       pos = decomp%x1disp(m) + 1
       istat = cudaMemcpyAsync( work2_r_d(pos), work2_r(pos), decomp%x1cnts(m), a2a_h2d )
...
call nvtxEndRange
  ...
  subroutine transpose_y_to_x_complex_d(src, dst, decomp)
    ...
    return
  end subroutine transpose_y_to_x_complex_d
#endif
