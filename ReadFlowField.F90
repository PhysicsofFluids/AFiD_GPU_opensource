...
!     integer (kind=MPI_ADDRESS_KIND) :: extent,lb
      real(fp_kind) :: stro3
      real(fp_kind), allocatable, dimension(:,:,:) :: tempold,vyold,vxold,vzold
      logical :: fexist
...
!RO   Interpolate
      ...
      xs2og = floor(real(xstart(2)*nyom/nym,fp_kind))
      xe2og = ceiling(real(xend(2)*nyom/nym,fp_kind))
      xs3og = floor(real(xstart(3)*nzom/nzm,fp_kind))
      xe3og = ceiling(real(xend(3)*nzom/nzm,fp_kind))
...
!EP   Normal one to one HDF read (no interpolation)
     ...
     & ,xstart(3),xend(3),teflag,temp)
      ...
      if (resetlogstime) time=real(0.,fp_kind)
